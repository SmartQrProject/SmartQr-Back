...
Before running the project you need to create a `.env.development` file in the
`smart-qr-app` root. Use the provided `.env.development.example` as a starting
point:
...
The variables loaded by [`src/config/env.loader.ts`](src/config/env.loader.ts)
include:
...
AUTH0_JWKS_URL=https://your-auth0-domain/.well-known/jwks.json
