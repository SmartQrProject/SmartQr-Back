...
export class CategoriesRepository {
  ...
  private async validateCategoryName(name: string, restaurantId: string): Promise<void> {
    ...
    if (existingCategory) {
      throw new ConflictException(`There is already a category named "${name}" in this restaurant`);
    }
  }
  ...
  async findAllByRestaurant(restaurantId: string, page: number = 1, limit: number = 10): Promise<{ categories: Category[]; total: number; page: number; limit: number }> {
    ...
    if (!categories.length) {
      throw new NotFoundException(`No categories were found for the restaurant with ID ${restaurantId}`);
    }
  ...
  async updateCategorySequences(categories: { id: string; sequenceNumber: number }[], restaurantId: string): Promise<void> {
    ...
    await Promise.all(categories.map((update) => this.categoryRepository.update({ id: update.id, restaurant: { id: restaurantId } }, { sequenceNumber: update.sequenceNumber })));
  }
}
