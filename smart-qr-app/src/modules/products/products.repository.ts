...
export class ProductsRepository {
  ...
  async findAllByRestaurant(restaurantId: string, page: number = 1, limit: number = 10): Promise<{ products: Product[]; total: number; page: number; limit: number }> {
  ...
  async updateProductSequences(products: { id: string; sequenceNumber: number }[], restaurantId: string): Promise<void> {
    ...
    await Promise.all(products.map((update) => this.productRepository.update({ id: update.id, restaurant: { id: restaurantId } }, { sequenceNumber: update.sequenceNumber })));
  }
}
